...
cameraMatrix = cv2.UMat(np.ascontiguousarray(np.array(data['cameraMatrix'], dtype=np.float32)))
distCoeffs = cv2.UMat(np.ascontiguousarray(np.array(data['distCoeffs'], dtype=np.float32).reshape((5,1))))
...
while True:
    ...
    for r in res:
        ...
        corners = r.corners
        corners = cv2.UMat(np.ascontiguousarray(np.array(corners, dtype=np.float32).reshape((4,2,1))))
        print(world_points[r.tag_id], corners, cameraMatrix, distCoeffs)
        retval, rvec, tvec = cv2.solvePnP(world_points, corners, np.eye(3), np.zeros((4,1)))
